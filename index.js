...
+ 'FN: BRUXINHO MODS WAR\n' // Nama

+ 'ORG:BRUXINHO MODS WAR;\n' // Nama bot
...
Modelo ${prefix}verify untuk para começar a usar bot.`
  ...
  only: {
    ...
    group: '*☒* Este recurso é para o jhon*',

    ownerG: '*☒* Este recurso é para *OWNER GRUPO*',

    ownerB: '*☒* Este recurso é para *JHON*',
...
if (getmemex <= memberlimit) {
  ...
  Pin.sendMessage(from, `Maaf syarat member harus di atas ${memberlimit}, selamat tinggal 👋🏻`, text)
...
//--Info bot
  ...
  ‣ *Proprietário* : Bruxinho MODS war
...
if (args.length < 1) return Pin.sendMessage(from, '*☒* Masukan teks', text, {
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								...
								exec(`webpmux -set exif ${addMetadata('BRUXINHO', 'CURA LÉSBICA KKKK')} ${ran} -o ${ran}`, async (error) => {
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								...
								exec(`webpmux -set exif ${addMetadata('BRUXINHO', 'CURA LÉSBICA KKK')} ${ran} -o ${ran}`, async (error) => {
